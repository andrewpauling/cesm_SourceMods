!|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
 ...
 module forcing_sfwf
! CMB added option for formulation to be hosing, which can be used in coupled mode
! Testing gave bfb (big for bit same answers) when code was compiled with all my
! changes but without setting the namelist option to turn on the hosing code
! hence no user_nl_pop2 options set. Then I also tested with hosing option turned on
! but hosing input file had all zeros, also gave gfb. These tests are
! important because the namelist variables are fussy and have
! potential interdependency.
! Probably should not hose in the marginal seas !!! 
! The model does some correcting of the net fw flux to
! ensure freshwater balance in each of the marginal seas (see set_forcing_coupled)
! I made this correcting first BEFORE adding hosing flux. So hosing if in the marginal 
! seas will destroy balance. Best just not to hose there. 
...
!
! !REVISION HISTORY:
!  SVN:$Id: forcing_sfwf.F90 25562 2010-11-10 23:36:51Z njn01 $
!
! !USES:
...
! !PUBLIC DATA MEMBERS:
   ...
   real (r8), public :: &! public for use in restart
      ...
      hosing_fact = c1, &! factor for adjusting hosing for water balance
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind), public :: &! hosing options
      sfwf_data_hosing,          &
      sfwf_comp_hosing
...
!-----------------------------------------------------------------------
   ...
   type (io_field_desc) :: &
      ...
      io_hosing,      &! io field descriptor for input hosing field
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
    write(stdout,'(a70)') &
        '   CMB added a new sfwf type called hosing, be sure to use it when coupled'
   endif
! CMB Note that turning the data_type to anything but 'none' turns on 
! CMB the code here. It would be bad to have 
! CMB the formulation be anything but 'hosing' when running coupled!!!
! CMB Note that hosing formumation does not use the data_renorm or any variable
! CMB with restore in it, make them one or zero for safety sake.
! CMB Model code ships with these here set to POP defaults and then practice has
! CMB been to change to CCSM defaults and customize for given run in the namelist
! CMB I changed to make these the CCSM defaults and customize for hosing here.
! CMB The namelist is still read after this and can alter these defaults

   sfwf_formulation       = 'hosing'
   sfwf_data_type         = 'none'
   sfwf_data_inc          = 1.e20_r8  ! CMB not used if data_type is monthly
   sfwf_interp_type       = 'linear'  ! CMB interpolation in time
   sfwf_interp_freq       = 'every-timestep' ! CMB how oftern to interpolate in time
   sfwf_interp_inc        = 1.e20_r8         ! CMB not used if doing every-timestep
   sfwf_restore_tau       = 1.e20_r8         ! CMB not used if 'hosing'
   sfwf_filename          = 'sfwftest2deg'
   sfwf_file_fmt          = 'nc'
   sfwf_data_renorm       = c1
   ladjust_precip         = .false.   ! CMB balances P globally, .false. in CCSM coupled default
   lms_balance            = .true.    ! CMB balance P,E,M,R,S in marginal seas, .true. in CCSM coupled default
   sfwf_weak_restore      = 0.0_r8    ! CMB make these zero to stop any restoring
   sfwf_strong_restore_ms = 0.0_r8
   sfwf_strong_restore    = 0.0_r8
   lfw_as_salt_flx        = .true.    ! CMB makes all fw fluxes a virtual salt flux, .true. in ccsm namelist
   lsend_precip_fact      = .false.   ! CMB this does nothing, might as well be false. supposed to be 
                                      ! a parameter sent by coupler when true
...
!-----------------------------------------------------------------------
   ...
   case ('hosing')
      ...
      sfwf_data_hosing = 1
      sfwf_num_comps = 2
      sfwf_comp_hosing = 1    ! put the hosing stuff in this index of SFWF_COMP
      sfwf_comp_cpl = 2       ! put the cpl stuff in this index of SFWF_COMP 

      allocate(sfwf_data_names(sfwf_data_num_fields), &
               sfwf_bndy_loc  (sfwf_data_num_fields), &
               sfwf_bndy_type (sfwf_data_num_fields))
      sfwf_data_names(sfwf_data_hosing) = 'HOSING'  ! CMB I think this sets up the variable name for nc input file
      sfwf_bndy_loc  (sfwf_data_hosing) = field_loc_center
      sfwf_bndy_type (sfwf_data_hosing) = field_type_scalar
   ...
   case ('annual')
      ...
      case ('hosing')
         ...
         io_hosing = construct_io_field( &
                       trim(sfwf_data_names(sfwf_data_hosing)),       &
                       dim1=i_dim, dim2=j_dim,                               &
                       field_loc = sfwf_bndy_loc(sfwf_data_hosing),   &
                       field_type = sfwf_bndy_type(sfwf_data_hosing), &
                       d2d_array=SFWF_DATA(:,:,:,sfwf_data_hosing,1))
   ...
   case ('monthly-equal','monthly-calendar')
      ...
      case ('hosing')
         io_hosing = construct_io_field( &
                       ...
                       field_loc = sfwf_bndy_loc(sfwf_data_hosing),   &
                       field_type = sfwf_bndy_type(sfwf_data_hosing), &
                       d3d_array=TEMP_DATA(:,:,:,:,sfwf_data_hosing))
         call data_set(forcing_file,'define',io_hosing)
         call data_set(forcing_file,'read'  ,io_hosing)
         call destroy_io_field(io_hosing)
      ...
      !*** re-order data and renormalize 
      !*** e.g. compensate for different units.
...
!-----------------------------------------------------------------------
   ...
   case ('n-hour')
! CMB WARNING, hosing cannot be n-hour
      ...
      do n=1,sfwf_interp_order
         ...
         select case (sfwf_formulation)
         case ('hosing')
           call exit_POP(sigAbort, 'swfw_formulation type hosing is not ' /&
              &/ 'coded to allow data_type of n-hour ' )
         case ('restoring')
            io_sss = construct_io_field( &
                       trim(sfwf_data_names(sfwf_data_sss)),       &
                       dim1=i_dim, dim2=j_dim,                               &
      ...
      do n = 1,sfwf_data_num_fields
         SFWF_DATA(:,:,:,n,:) = sfwf_data_renorm(n)*SFWF_DATA(:,:,:,n,:)
      enddo
...
! CMB CCSM expects tracer concentration (TFW) to be zero
   real (r8), dimension(nx_block,ny_block,nt,max_blocks_clinic), &
      intent(inout) :: &
      STF,    &! surface tracer fluxes at current timestep
...
!-----------------------------------------------------------------------
   ...
   case('annual')
      ...
      case ('hosing')
         call calc_sfwf_hosing(1) ! case of one (annual mean) hosing flux for all time
   ...
   case ('monthly-equal','monthly-calendar')
      ...
      case ('hosing')
         ...
         call calc_sfwf_hosing(12)  ! CMB input 12 months of hosing, interpolate to the model time
   ...
   case('n-hour') ! CMB not gonna worry about making this option for hosing
...
!***********************************************************************
!BOP
! !IROUTINE: calc_sfwf_hosing
! !INTERFACE:

 subroutine calc_sfwf_hosing(time_dim)
! CMB written by me
! Hosing is tricky in the sense that it is computed on a different time step
! than that of the coupler calling pop_set_forcing_coupled, which computes
! the forcing from the vars sent by the coupler. Use same method as partial-coupled
! that is to just put the hosing in SFWF_COMP index sfwf_comp_hosing
! and later combine with the coupler terms in forcing.F90 subroutine sfwf_combined_forcing


! !DESCRIPTION:
!  Calculates surface freshwater flux from hosing, akin to adding
!  additional precipitation
!
!  Notes:
!    the forcing data (on t-grid) are computed and
!    stored in SFWF\_DATA(:,:,sfwf\_comp\_*,now) where:
!       sfwf\_data\_hosing is   like precipitation       (m/y)
!
! !REVISION HISTORY:
!  same as module
...
!-----------------------------------------------------------------------
   ...
   stf_sum1 = global_sum(SFWF_COMP(:,:,:,sfwf_comp_hosing),distrb_clinic,field_loc_center)

   !$OMP PARALLEL DO PRIVATE(iblock, this_block)
   do iblock=1,nblocks_clinic
      this_block = get_block(blocks_clinic(iblock),iblock)

      ! *** SFWF_COMP at the end of this step should have units of kg/m2/s
      ! *** hosing_fact is 1 at present since the input file should have units of kg/m2/s

      SFWF_COMP(:,:,iblock,sfwf_comp_hosing) = &
         SFWF_DATA(:,:,iblock,sfwf_data_hosing,now)*hosing_fact 

      ! *** SFWF_COMP at the end of this step should have units of msu*cm/s
      ! *** same as STF(:,:,2,:) is subroutine pop_set_coupled_forcing of forcing_coupled.F90
      ! *** AGP tried multiplying here by function of tyear to allow nonlinear ramping.

      SFWF_COMP(:,:,iblock,sfwf_comp_hosing) = &
         SFWF_COMP(:,:,iblock,sfwf_comp_hosing)*salinity_factor*2000/0.3863*log(tyear+1)

   end do ! block loop
   !$OMP END PARALLEL DO

   stf_sum2 = global_sum(SFWF_COMP(:,:,:,sfwf_comp_hosing),distrb_clinic,field_loc_center)

   if (my_task == master_task) then
    write(stdout,'(a30,2(e12.3))') &
        'HOSING SFWF_COMP global sums 1 and 2 ', stf_sum1, stf_sum2
   endif

!-----------------------------------------------------------------------
!EOC

 end subroutine calc_sfwf_hosing
