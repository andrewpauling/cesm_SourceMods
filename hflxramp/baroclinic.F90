...
!   use distribution, only: 
   ...
   use forcing_fwf_interior, only: set_fwf_interior
   use forcing_hf_interior, only: set_hf_interior
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_IFW,        &! tavg id for ifw  CMB
      tavg_dSdt,       &! tavg id for ifw  CMB
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_IFW,'IFW',3,                              &
                          long_name='Interior Salt Flux',            &
                          units='gram/gram/s', grid_loc='3111',     &
                          coordinates='TLONG TLAT z_t time')

   call define_tavg_field(tavg_dSdt,'dSdt',3,                              &
                          long_name='Interior Salt Tendency',            &
                          units='gram/gram/s', grid_loc='3111',     &
                          coordinates='TLONG TLAT z_t time')
...
! use temporary variable WORKIFW here instead of WORKN to make diagnostic
   WORKIFW = c0  ! CMB must zero since set_fwf_interior adds to this variable
   call set_fwf_interior (k,this_block,WORKIFW) ! CMB 
   call accumulate_tavg_field(WORKIFW,tavg_IFW,bid,k) !CMB
   WORKN(:,:,2)=WORKN(:,:,2)+WORKIFW ! CMB finally add IFW to dS/dt source terms

! AGP add in temperature tendency from heat flux to first index of WORKN
   call set_hf_interior(k,this_block,WORKN(:,:,1))  

! CMB note nt is the number of tracers, and the 2nd index is for salt
! CMB hence this is not where the fwf_interior flux gets added
...
!CMB note here our SOURCE terms get absorbed into the 
!CMB variable for the dT/dt terms of the tracer equation
   FT = FT + WORKN

   WORKdSdt = FT(:,:,2) - WORKIFW ! CMB make diagnostic for all dS/dt except IFW 
   call accumulate_tavg_field(WORKdSdt,tavg_dSdt,bid,k)
