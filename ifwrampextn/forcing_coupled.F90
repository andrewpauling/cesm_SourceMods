...
!
! !REVISION HISTORY:
!  SVN:$Id: forcing_coupled.F90 28439 2011-05-18 21:40:58Z njn01 $
!
! !USES:
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      tavg_HOSE_F,       &! tavg id for hosing flux
...
!-----------------------------------------------------------------------
   ...
   if ( sfwf_formulation == 'hosing') then  !CMB
      call define_tavg_field(tavg_HOSE_F,'HOSE_F',2,                           &
                          long_name='Hosing Flux read into POP',               &
...
!-----------------------------------------------------------------------
   ...
   if (lcoupled .and. sfwf_formulation /= 'partially-coupled' .and.  &
       sfwf_formulation /= 'hosing' .and.  sfwf_data_type /= 'none') then ! CMB 
     message =   &
...
! CMB the routine is called by ocn_comp_mct
!
! !REVISION HISTORY:
!  same as module
...
!-----------------------------------------------------------------------
   ...
   real (r8) :: stf_sum1, stf_sum2
   real (r8) :: rcalct1, rcalct2, psum1, psum2, esum1, esum2, msum1, msum2
   real (r8) ::  rsum1, rsum2, isum1, isum2, ssum1, ssum2
...
!  (CMB notes that lfw_as_salt_flx is T in CCSM so this is the correct part)
!
!-----------------------------------------------------------------------

! CMB find global sum of STF before adding coupler fluxes
      stf_sum1 = global_sum(STF(:,:,2,:),distrb_clinic,field_loc_center)
!      rcalct1 = global_sum(RCALCT,distrb_clinic,field_loc_center)
!      psum1 = global_sum(PREC_F,distrb_clinic,field_loc_center)
!      esum1 = global_sum(EVAP_F,distrb_clinic,field_loc_center)
!      msum1 = global_sum(MELT_F,distrb_clinic,field_loc_center)
!      rsum1 = global_sum(ROFF_F,distrb_clinic,field_loc_center)
!      isum1 = global_sum(IOFF_F,distrb_clinic,field_loc_center)
!      ssum1 = global_sum(SALT_F,distrb_clinic,field_loc_center)
...
! CMB find global sum of STF after adding coupler fluxes
      stf_sum2 = global_sum(STF(:,:,2,:),distrb_clinic,field_loc_center)

   if (my_task == master_task) then
    write(stdout,'(a30,2(e12.3))') &
        'COUPLED STF FW global sums 1 and 2 ', stf_sum1, stf_sum2
!    write(stdout,'(9(e12.3))') &
!        rcalct1,psum1,esum1,msum1,rsum1,isum1,ssum1,salinity_factor,sflux_factor
   endif
...
!
!-----------------------------------------------------------------------
! CMB note do this even if hosing since presumably if you hose in the marginal
! seas, you want them to be out of balance, even though you set lms_balance to T
   ...
   do iblock = 1, nblocks_clinic
      ...
      else if ( sfwf_formulation == 'hosing' ) then   ! CMB added

        if (sfc_layer_type == sfc_layer_varthick .and.  &
            .not. lfw_as_salt_flx) then
          ! CMB I am going to avoid this condition since we actually want
          ! CMB lfw_as_salt_flx to be true. If one were to change so lfw_as_salt_flx 
          ! CMB is false, this might work but would still need much add'n coding elsewhere
          ! CMB Just die for now if ever get here
          call exit_POP(sigAbort,'ERROR hosing should have lfw_as_salt_flx be true')
          SFWF_COMP(:,:,iblock,sfwf_comp_cpl) =  FW(:,:,iblock) 
             do n=1,nt
              TFW_COMP(:,:,n,iblock,tfw_comp_cpl) = TFW(:,:,n,iblock) 
             enddo
        else
...
!-----------------------------------------------------------------------
  ...
  real (r8) stf_sum2
   ...
   if (my_task == master_task) then
    write(stdout,'(a70)') &
        'In subroutine set_combined_forcing'
   endif
   ...
   else if ( sfwf_formulation == 'hosing' ) then
     if (sfc_layer_type == sfc_layer_varthick .and.  &
         .not. lfw_as_salt_flx) then
...
! CMB note the balancing of freshwater in marginal seas is done above in set_forcing_coupled
! irrespective of hosing. Hence hosing can break balance if there is any in the marginal seas
! even if you set lms_balance to be T
         !$OMP PARALLEL DO PRIVATE(iblock)
         do iblock=1,nblocks_clinic
           STF(:,:,2,iblock) =  SFWF_COMP(:,:,iblock,sfwf_comp_cpl)    &
   ...
   if (my_task == master_task) then
    write(stdout,'(a70)') &
        'STF is combined coupler and hosing forcing'
!    stf_sum2 = global_sum(SFWF_COMP(:,:,:,sfwf_comp_hosing),distrb_clinic,field_loc_center)
!    write(stdout,'(a30,e12.3)') &
!        'HOSING SFWF_COMP in combined forcing ',  stf_sum2
   endif
 
     endif ! if surf_layer_blah blah

   endif   ! if sfwf_formulation == various things
...
!-----------------------------------------------------------------------
   ...
   do iblock = 1,nblocks_clinic
      ...
      this_block = get_block(blocks_clinic(iblock),iblock)
         if ( sfwf_formulation == 'hosing') then  !CMB
           WORK = SFWF_COMP(:,:,iblock,sfwf_comp_hosing)/salinity_factor
           call accumulate_tavg_field(WORK, tavg_HOSE_F,iblock,1)
	 endif
